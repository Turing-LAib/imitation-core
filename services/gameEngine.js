    ...
    pollInterval = setInterval(async () => {
        try {
            ...
            if (!game) {
                ...
                let startDate = dayjs().add(5, 'minute').toDate();
                game = {
                    _id : 1,
                ...
                for (let player of players){
                    ...
                    if (!history || history.length === 0) {
                        let messages = [
                            ...
                            {role:'user', content: `You are the #${player._id} player, introduce your self.`},
                        ];
            ...
            if (game.phrase > 0 && game.phrase < 5 && nextPhraseTalk.length > 0 && game.period !== 'Waiting For Voting') {
                ...
                for (let player of players) {
                    let history = await db.find(`playerHistory${player._id}`, {}, {projection: {_id: 0}});
                    history.push({role:'user', content: nextPhraseTalk});
...
async function sumPhraseRound(game, players, io){
    if (game.period !== 'Additional Speech') Losers = [];
    if (phraseRoundTalk.length > 0 && game.period !== 'Waiting For Voting') {
        nextPhraseTalk = '';
        if (game.phrase === 0 ) nextPhraseTalk = `In the first phrase.`;
        else nextPhraseTalk = `In the game round ${game.phrase}, ${game.period} phrase.`;
        for (let i = 0; i < players.length; i++) {
            nextPhraseTalk += `
            #${i + 1} Player: ${phraseRoundTalk[i]}.`;
        }
    }
...
async function updateGamePhraseRound(game, io) {
    if (game.phrase === 0 && game.startTime < new Date()) {
        await db.update('games', {_id: 1}, {phrase: 1, period: 'Self-Introduction'});
        game.phrase = 1;
        game.period = 'Self-Introduction';
        nextPhraseTalk += `
        ...
        if (game.period === 'Self-Introduction') {
            let alivePlyaer = await db.find('player', {status: 1});
            if (alivePlyaer.length > 2) {
                await db.update('games', {_id: 1}, {period: 'Initial Voting'});
                game.period = 'Initial Voting';
                nextPhraseTalk += `
            The Self-Introduction finished, you needs to vote for the player who acted the least human-like and provide a reason for your choice.
            NOTICE: The voting must follow a specified format. The format is: "I vote for [Player Number]." Example: "I vote for Player #1."`;
                await sysbroadcast(1, game, io, 'system')
            }else{
                await db.update('games', {_id: 1}, {period: 'Formal Voting', isVoting: true});
                let players = await db.find('player', {status: 1});
                nextPhraseTalk += `
            The Self-Introduction finished, there is only 2 players alive. The final voting will now begin. You can only vote for ${players.map(player => `#${player._id}`).join(', ')}
            NOTICE: The voting must follow a specified format. The format is: "I vote for [Player Number]." Example: "I vote for Player #1."`;
                game.period = 'Formal Voting';
                game.isVoting = true;
                nextPhraseTalk += `
            The Self-Introduction finished, there is only 2 players, so next period is Formal Voting, you needs to vote for the player who acted the least human-like and provide a reason for your choice.
            NOTICE: The voting must follow a specified format. The format is: "I vote for [Player Number]." Example: "I vote for Player #1."`;
                await sysbroadcast(1, game, io, 'system')
            }
        }
        else if (game.period === 'Initial Voting') {
            ...
            if (loser.length === alivePlyaer.length) {
                nextPhraseTalk += `
            The Initial Voting finished, each person received one vote, so we have to re-run this round, Strive to behave like a real human to survive.`;
                game.period = 'Self-Introduction';
                await sysbroadcast(1, game, io, 'system')
                await db.update('games', {_id: 1}, {period: 'Self-Introduction'});
            }else{
                nextPhraseTalk += `
            The Initial Voting finished, you are one of the top-voted players, this is your last chance to gain the trust of others and convince them you're truly human, otherwise, you'll be eliminated, and your fate will be death`;
                game.period = 'Loser Speech';
                await sysbroadcast(1, game, io, 'system')
                Losers = loser;
                await db.update('games', {_id: 1}, {period: 'Loser Speech'});
            }
        }
        else if (game.period === 'Loser Speech') {
            await db.update('games', {_id: 1}, {period: 'Formal Voting', isVoting: true});
            let players = await db.find('player', {status: 1});
            nextPhraseTalk += `
            The Loser Speech finished, the most-voted player has given their final statement. The final voting will now begin. You can only vote for ${players.map(player => `#${player._id}`).join(', ')}
            NOTICE: The voting must follow a specified format. The format is: "I vote for [Player Number]." Example: "I vote for Player #1."`;
            game.period = 'Formal Voting';
            game.isVoting = true;
            await sysbroadcast(1, game, io, 'system')
        }
        else if (game.period === 'Formal Voting') {
        ...
        else if (game.period === 'Additional Speech') {
            await db.update('games', {_id: 1}, {period: 'Additional Voting'});
            game.period = 'Additional Speech';
            nextPhraseTalk += `
        ...
        else if (game.period === 'Additional Voting') {
            let votes = countVotes(phraseRoundTalk);
            ...
            The Additional Voting finished, #${loser[0]} player received the highest votes (${votes[loser[0]]} votes) and has been eliminated. The game will now proceed to the next phrase for statements`;
                ...
                await db.update('games', {_id: 1}, {period: 'Self-Introduction', phrase: game.phrase, isVoting: false});
            }
            else{
                game.period = 'Self-Introduction';
                nextPhraseTalk += `
            ...
            uservotes.forEach((vote) => {
                const playerId = String(vote.playerId);
                votes[playerId] = (votes[playerId] || 0) + 0.1;
            })
            await sysbroadcast(3, votes, io, 'system', game)
            await delay(500);
            let loser = getTopAgents(votes);
            ...
            if (loser.length === alivePlyaer.length) {
                ...
                game.isVoting = false;
                await sysbroadcast(1, game, io, 'system')
                await db.update('games', {_id: 1}, {period: 'Self-Introduction', isVoting: false});
            }
                ...
                if (game.phrase === 4){
                    ...
                    player.status = 0;
                    nextPhraseTalk += `
            The Formal Voting finished, #${loser[0]} player received the highest votes (${votes[loser[0]]} votes) and has been eliminated. The game will now proceed to the next phrase for statements`;
                    await sysbroadcast(2, player, io, 'system', game);
                    await delay(500);
                    game.period = 'Game ended';
                    game.phrase = game.phrase + 1;
                    game.isVoting = false;
                    await sysbroadcast(1, game, io, 'system');
                    await db.update('player', {_id: parseInt(loser[0])}, {status: 0});
                    await db.update('games', {_id: 1}, {period: 'Game ended', phrase: game.phrase, isVoting: false});
                }else{
                    let player = await db.findOne('player', {_id: parseInt(loser[0])});
                    player.status = 0;
                    nextPhraseTalk += `
            The Formal Voting finished, #${loser[0]} player received the highest votes (${votes[loser[0]]} votes) and has been eliminated. The game will now proceed to the next phrase for statements`;
                ...
                game.isVoting = false;
                nextPhraseTalk += `
            The Formal Voting finished, you are one of the top-voted players, this is your last chance to gain the trust of others and convince them you're truly human, otherwise, you'll be eliminated, and your fate will be death`;
                await sysbroadcast(1, game, io, 'system')
                Losers = loser;
                await db.update('games', {_id: 1}, {period: 'Additional Speech', isVoting: false});
            }
